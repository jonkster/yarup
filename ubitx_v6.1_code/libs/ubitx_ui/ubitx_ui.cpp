...
int getValueByKnob(int minimum, int maximum, int step_size,  int initial, char* prefix, char *postfix)
{
	...
	while(!encoderBtnDown() && digitalRead(PTT) == HIGH) {
...
void fastTune() {
	...
	while(1) {
		...
		//exit after debouncing the encoderBtnDown
		if (encoderBtnDown()) {
...
void setCwTone() {
	...
	while (digitalRead(PTT) == HIGH && !encoderBtnDown())
	{
		knob = getEncoderDir();
